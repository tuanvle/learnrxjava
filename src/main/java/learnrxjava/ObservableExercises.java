...
//        return Observable.error(new RuntimeException("Not Implemented"));
  ...
  public Observable<String> exerciseMap(Observable<String> hello) {
    return hello.map(s -> "Hello " + hello + "!");
...
//    return Observable.error(new RuntimeException("Not Implemented"));
    return nums.reduce((max, item) -> item > max ? item : max);
  }
    ...
    return movies.flatMap(m -> {
      return m.videos.flatMap(v -> {
        return v.boxarts.reduce((min, box) -> {
          long minsize = min.height * min.width;
          long size = box.height * box.width;

          return size < minsize ? box : min;
        }).map(smallestBox -> json("id", v.id, "title", v.title, "boxart", smallestBox.url));
      });
    });
  }
