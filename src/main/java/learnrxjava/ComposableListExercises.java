...
//        throw new UnsupportedOperationException("Not implemented yet.");
  ...
  public static ComposableList<Integer> exercise8() {
    ...
    return newReleases.filter(r -> r.rating == 5.0)
        .map(v -> v.id);
...
//        throw new UnsupportedOperationException("Not implemented yet.");
  ...
  public static ComposableList<Integer> exercise9() {
    ...
    movieLists.forEach(l -> {
      l.videos.forEach(v -> allVideoIdsInMovieLists.add(v.id));
    });
    return allVideoIdsInMovieLists;
//        throw new UnsupportedOperationException("Not implemented yet.");
  }
  ...
  public <R> ComposableList<R> concatMap(
    ...
    for (T itemInList : this) {
      ...
      projectionFunctionThatReturnsList.apply(itemInList).forEach(v -> results.add(v));
...
//        throw new UnsupportedOperationException("Not implemented yet.");
  ...
  public static ComposableList<Integer> exercise11() {
    ...
    return movieLists.concatMap(l -> l.videos.map(v -> v.id));
//        throw new UnsupportedOperationException("Not implemented yet.");
  }
...
//        return movieLists.concatMap(l -> {
//            return l.videos.concatMap(v -> {
//                return v.boxarts.filter(b -> b.width == 150 && b.height == 200)
//                    .map(b -> json("id", v.id, "title", v.title, "boxart", b.url));
//            });
//        });

    return movieLists.concatMap(l ->
        l.videos.concatMap(v -> v.boxarts
...
//    throw new UnsupportedOperationException("Not implemented yet.");
  ...
  */
  public static ComposableList<Integer> exercise16() {
    ...
    return ratings.reduce((max, rating) -> rating > max ? rating : max);
//    throw new UnsupportedOperationException("Not implemented yet.");
  }
  ...
  public static ComposableList<String> exercise17() {
    ...
    return boxarts.reduce((maxBox, box) -> {
      long maxSize = maxBox.width * maxBox.height;
      long size = box.width * box.height;
      return size > maxSize ? box : maxBox;
    }).map(b -> b.url);
//    throw new UnsupportedOperationException("Not implemented yet.");
  }
  ...
  public static ComposableList<JSON> exercise19() {
    ...
    return movieLists.concatMap(l -> l.videos
        .concatMap(v -> v.boxarts
            .reduce((smallest, box) -> {
              long smallestSize = smallest.width * smallest.height;
              long size = box.width * box.height;
              return size < smallestSize ? box : smallest;
            })
            .map(smallest -> json("id", v.id, "title", v.title, "boxart", smallest.url))
        )
    );
//    throw new UnsupportedOperationException("Not implemented yet.");
  }
...
//    throw new UnsupportedOperationException("Not implemented yet.");
  ...
  public static ComposableList<JSON> exercise23() {
    ...
    return movieLists.concatMap(
        l -> l.videos.concatMap(v ->
            ComposableListExercises.zip(
                v.boxarts.reduce((smallest, box) -> {
                  long smallestSize = smallest.width * smallest.height;
                  long size = box.width * box.height;
                  return size < smallestSize ? box : smallest;
                }),
                v.interestingMoments.filter(m -> m.type.equals("Middle")),
                (b, m) -> json("id", v.id, "title", v.title, "time", m.time, "url", b.url)
            )
        )
    );
